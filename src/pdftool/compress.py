...
def _resolve_level(level: int | str | None) -> str | None:
    if level is None:
        return ask_compress_level()
    if isinstance(level, int):
        if level not in _LEVEL_BY_INT_:
            raise ConversionError(f"Invalid compression level: {level!r}. Use 1 (Low), 2 (Medium), or 3 (High).")
        return _LEVEL_BY_INT_[level]
    if level not in _IMG_QUALITY:
        raise ConversionError(f"Invalid compression level: {level!r}. Use 'low', 'medium', or 'high'.")
    return level


def pdf_compres(path: Path, level: int | str | None = None, output: Path | None = None):
...
def jpg_compres(path: Path, level: int | str | None = None, output: Path | None = None):
    level = _resolve_level(level)
    if level is None:
        return
